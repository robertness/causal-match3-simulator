...
def test_trajectory_reuses_one_skill_and_has_no_pre_landmark_churn() -> None:
    ...
    assert all(
        record.churn_probability == 0.0 and record.churn_after == 0
        for record in trajectory.attempts[:-1]
    )


def test_trajectory_stops_at_first_landmark_churn() -> None:
    trajectory = simulate_player_trajectory(
        _model(),
        player_id=11,
        seed=223,
    ...
    assert trajectory.churn_attempt == BENCHMARK_CONFIG.landmark_attempt
    assert len(trajectory.attempts) == BENCHMARK_CONFIG.landmark_attempt
